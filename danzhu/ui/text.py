...
_GLYPH_ON = True                 # 总开关。⚠️ 2026-09-20 打开 —— 桌面探针已把当初关闭的理由
                                 #    ("未解释的 1 像素")**量掉了**: 4 档字号下
                                 #    `tex.size == content_size == (逐字宽之和, 图集高)`, 且整串
                                 #    拼出来与老路纹理**逐像素 0 差异**(见 `tests/fx_gates.py` 那条
                                 #    逐像素门禁)。⇒ 那个像素应该是后来"必须整串烘再切"那次修复
                                 #    顺带解决的, 只是开关没跟着打开。
                                 #    ⚠️ 安全网是真的: `_glyph_bake` 三道门禁任一不过 ⇒ 返回 None
                                 #    ⇒ 该档永不启用, 逐字退回普通 `Label`。
