...
def main():
    ...
    # =====================================================================
    # [31] 球纹理分块烘焙: 切了之后**出图必须逐字节相同**
    # ⚠️ 2026-09-20: 真机启动日志里 4 档球纹理是 **61~63 ms 一帧**, 而它们发生在
    #    「加载页摘了 3.7 秒之后」(玩家已经在游戏里)。⇒ 把逐像素循环按行切块。
    #    **这类"把整块活切碎"的改动最怕的就是切出偏差** —— 所以这里逐字节对账,
    #    而不是只看"跑起来了没报错"。
    # =====================================================================
    print("\n[31] 球纹理分块烘焙: 切块 vs 整块必须**逐字节相同**")

    _D = WFX._CUP_BALL_TEX_PX
    _R = WFX._BALL_TEX_ROWS
    _bad = []
    for _bet in (1, 10, 50, 100, 999):          # 999 = 未知档回退金球那条分支
        _j1 = WFX._bake_begin(_bet)
        WFX._bake_advance(_j1, 2 * _D)          # 一次算完(= 改造前的行为)
        _j2 = WFX._bake_begin(_bet)
        _n = 0
        while WFX._bake_advance(_j2, _R) < 2 * _D:
            _n += 1
            if _n > 200:
                break
        if bytes(_j1["buf"]) != bytes(_j2["buf"]):
            _bad.append(_bet)
    check(not _bad,
          "分块烘焙与整块烘焙**逐字节相同**(5 档: 1/10/50/100 + 未知档回退)",
          "每步 %d 行 · 一档 %d 步 · 不一致的档: %s" % (_R, (2 * _D + _R - 1) // _R, _bad or "无"))

    # 单步成本必须**留得住余量**: 一帧预算 6.06ms(165Hz), 正常 body 约 1.3ms。
    # ⚠️ 这条只是**量级门槛**(桌面比真机快、且这里没跑真机), 防的是"有人把 _BALL_TEX_ROWS 调回 256"。
    check(_R * 2 < _D + _D,          # 行数必须真的切开了, 不是一步到位
